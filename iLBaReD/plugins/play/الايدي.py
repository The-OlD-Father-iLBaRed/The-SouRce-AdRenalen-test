...
The_Stayle = [ "😂♥️ وربنا جامد يعم رقمك زيرو كام ♥️😂","علفكرا صورتي احسن منك 😂♥️","الجمال عدا الخيال 😂♥️","متراعي ي لبن المراعي ♥️😂","مش ناوي تغير الصوره دي بقا ♥️😂","غيرها بقا لما بشوفها بتخض"]
ID_MEMBARS = []
id = {}
...
async def iddlock(client: Client, message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
        if message.chat.id in ID_MEMBARS:
            return await message.reply_text("الايدي معطل من قبل  😋♥️ ،")
        ID_MEMBARS.append(message.chat.id)
...
async def iddopen(client: Client, message):
    get = await client.get_chat_member(message.chat.id, message.from_user.id)
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
        if message.chat.id not in ID_MEMBARS:
            return await message.reply_text("الايدي مفعل من قبل 😋♥️ ،")
        ID_MEMBARS.remove(message.chat.id)
        return await message.reply_text("تم تعطيل الايدي بنجاح 😋♥️ ،")
    else:
        return await message.reply_text("عذرا عزيزي هذا الامر للمشرفين بس 😋♥️ ،")
...
async def heart(client, query: CallbackQuery):
    ...
    caption = f" {choice(The_Stayle)} \n\n𝅄 𓏺 𝐂𝐇𝐀𝐓 𝐍𝐄𝐌 » ⦗ {query.message.chat.title} ⦘ 🕷 ⋅\n𝅄 𓏺 𝐍𝐄𝐌 » ⦗ {user.first_name} ⦘ 🕷 ⋅\n𝅄 𓏺 𝐔𝐒𝐄𝐑 𝐍𝐄𝐌 » ⦗ @{user.username} ⦘ 🕷 ⋅\n𝅄 𓏺 𝐁𝐈𝐎 » ⦗ {user.bio} ⦘ 🕷 ⋅\n𝅄 𓏺 𝐈𝐃 » ⦗ {query.message.chat.id} ⦘ 🕷 ⋅\n𝅄 𓏺 𝐂𝐇𝐀𝐓 𝐈𝐃 » ⦗ {query.message.chat.id} ⦘ 🕷 ⋅"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton(f"{idd} ♥️", callback_data=f"heart{user_id}")]])
